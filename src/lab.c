...
void Caminho(char **mat, int tam)
{
	...
	do
	{
		...
		if ((contador0 == contador1*tam*tam) && (contador1!=0) && contador1!=1){	
			vitoria = true;
			break;
		}
	...
	printf("\nRESULTADO DA MATRIZ NO METODO RANDOMICO: \n\n");
	for(int i=0; i<tam; i++){
			for(int j=0; j<tam; j++){
				printf("%c ", mat[i][j]);
			}
...
void DFS(char **mat, int tam, CoordenadaStack c)
{
	...
	printf("\nRESULTADO DA MATRIZ NO METODO DFS: \n\n");
	for(int i=0; i<tam; i++){
		for(int j=0; j<tam; j++){
				printf("%c ", mat[i][j]);
		}
...
void BFS(char **mat, int tam, CoordenadasQueue q)
{
	...
	printf("\nRESULTADO DA MATRIZ NO METODO BFS:\n\n");
	for(int i=0; i<tam; i++){	
		for(int j=0; j<tam; j++){
			printf("%c ", mat[i][j]);
		}
	...
	printf("SENDO [0] O CAMINHO FEITO E [X] AS POSICOES ENFILEIRADAS NAO VISITADAS\n");
